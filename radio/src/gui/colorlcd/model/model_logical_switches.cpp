...
class LogicalSwitchButton : public ListLineButton
{
 public:
  ...
  static constexpr LayoutVal FN_X = lvAdd(NM_W, NM_X + PAD_TINY);
  static constexpr LayoutVal FN_Y = NM_Y;
  static LAYOUT_SIZE_SCALED(FN_W, 50, 58)
  static constexpr LayoutVal V1_X = lvAdd(lvAdd(FN_X, FN_W), PAD_TINY);

  static constexpr LayoutVal V1_Y{NM_Y.l, 0};
  static LAYOUT_VAL_SCALED(V1_W, 88)
  static constexpr LayoutVal V2_X = lvAdd(V1_X, V1_W + PAD_TINY);
  static constexpr LayoutVal V2_Y = V1_Y;
  static constexpr coord_t AND_W = V1_W;
  static LAYOUT_SIZE_SCALED(DUR_W, 40, 54)
  static constexpr LayoutVal DEL_W = DUR_W;
  static constexpr LayoutVal AND_X =
      lvSub(lvSub(lvSub(ListLineButton::GRP_W, DUR_W), DEL_W),
            PAD_BORDER * 2 + AND_W + PAD_TINY * 3);
  static LAYOUT_SIZE_SCALED(AND_Y, 4, 20)
  static constexpr LayoutVal V2_W = lvSub(lvSub(AND_X, V2_X), PAD_TINY);
#else
...
#endif
#if defined(LCD_RUNTIME_LAYOUT)
  static constexpr LayoutVal DUR_X =
      lvSub(lvSub(lvSub(ListLineButton::GRP_W, DUR_W), DEL_W),
            PAD_BORDER * 2 + PAD_TINY * 2);
  static constexpr LayoutVal DUR_Y = AND_Y;
  static constexpr LayoutVal DEL_X =
      lvSub(lvSub(ListLineButton::GRP_W, DEL_W), PAD_BORDER * 2 + PAD_TINY);
  static constexpr LayoutVal DEL_Y = AND_Y;
#else
