...
#if defined(LCD_RUNTIME_LAYOUT) && !defined(BOOT)

#define GRP_H_L(n) ((QuickMenuGroup::QM_BUTTON_HEIGHT + PAD_MEDIUM) * n - PAD_MEDIUM + PAD_OUTLINE * 2)
#define GRP_H_P(n) ((QuickMenuGroup::QM_BUTTON_HEIGHT + PAD_SMALL) * n - PAD_SMALL + PAD_OUTLINE * 2)
#define GRP_H(n) (lcdIsLandscape() ? GRP_H_L(n) : GRP_H_P(n))
#elif LANDSCAPE
#define GRP_H(n) ((QuickMenuGroup::QM_BUTTON_HEIGHT + PAD_MEDIUM) * n - PAD_MEDIUM + PAD_OUTLINE * 2)
#define GRP_H_L(n) GRP_H(n)
#define GRP_H_P(n) GRP_H(n)
#else
...
class QuickMenu : public NavWindow
{
 public:
  ...
  static LAYOUT_ORIENTATION(QM_MAIN_ROWS, 3, 4)
#if defined(LCD_RUNTIME_LAYOUT)

  static constexpr LayoutVal QM_MAIN_W{GRP_W(5), GRP_W(3)};
  static constexpr LayoutVal QM_MAIN_H{GRP_H_L(3), GRP_H_P(4)};
  static constexpr LayoutVal QM_W = lvAdd(QM_MAIN_W, PAD_LARGE * 2);
  static constexpr LayoutVal QM_H =
      lvAdd(QM_MAIN_H, EdgeTxStyles::UI_ELEMENT_HEIGHT + PAD_MEDIUM * 2);
  static constexpr LayoutVal QM_MAIN_X = lvDiv(lvSub(QM_W, QM_MAIN_W), 2);
#else
  ...
  static constexpr LayoutVal QM_X =
      lvDiv(lvSub(LayoutVal{LCD_MAX_W, LCD_PHYS_W}, QM_W), 2);
  static constexpr LayoutVal QM_Y =
      lvDiv(lvSub(LayoutVal{LCD_PHYS_W, LCD_PHYS_H}, QM_H), 2);
#else
  ...
  bool builtLandscape = false;

  const void* builtStrings = nullptr;
#endif
